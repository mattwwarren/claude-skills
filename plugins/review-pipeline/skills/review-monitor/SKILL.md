---
name: review-monitor
description: Monitor PRs from first review through merge. Polls threads, performs delta reviews on new pushes, approves when all threads addressed, nudges idle authors. Use when actively shepherding a PR. Triggers on "monitor PR", "watch this PR", "/review-monitor".
---
...
When running from the checked-out `global-claude` repo or a Codex wrapper, prefer repo-relative script paths like `scripts/review_monitor.py`. The `${CLAUDE_PLUGIN_ROOT}/scripts/...` examples below remain valid installed-path fallbacks.
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py consume-pending
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py status --json
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py check <pr_number>
```
...
3. Act on each verdict:
   - **ADDRESSED** → mark the thread resolved:
     ```bash
     ${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py confirm-thread <pr_number> --repo <repo> --thread <thread_id>
     ```
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py complete <pr_number> --reason approved
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py record-nudge <pr_number>
```
...
<your-local-notify-script> \
  ...
  "Review Monitor" \
  red
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py mark-notified <pr_number> \
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py mark-comment-review <pr_number> \
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py record-auto-fix <pr_number> --repo <repo>
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/notify_escalation.sh "<message>"
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py mark-escalated <pr_number> --repo <repo>
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py update-slack-cursor <pr_number> \
  --repo <repo> --last-seen-ts <newest returned ts>
```

**First-cycle burst prevention:** On a freshly-upgraded state file run `${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py catchup` before the first cycle to avoid N concurrent pings.
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/review_monitor.py record-channel-bump <pr_number> --repo <repo>
```
